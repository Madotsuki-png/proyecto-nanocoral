...
export default function ProductDetail() {
  ...
  if (loading) return <div className="text-center py-20 text-neutral-500 text-xs">Cargando ficha técnica...</div>;
  if (!producto) return <div className="text-center py-20 text-xs text-red-400">Producto no encontrado.</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="grid md:grid-cols-2 gap-10 items-center bg-neutral-950 p-6 rounded-xl border border-neutral-800 mb-12">
        <div className="w-full h-72 bg-neutral-900 rounded-lg flex items-center justify-center text-neutral-600 border border-neutral-900 text-xs">
          [ Imagen: {producto.imagen_url} ]
        </div>

        <div>
          <span className="text-[10px] text-teal-400 font-mono tracking-widest uppercase">NanoCoral Premium Stock</span>
          ...
          <p className="text-xs text-neutral-400 mt-4 leading-relaxed">
            {producto.descripcion || 'Ejemplar o equipo seleccionado bajo controles estrictos para asegurar el correcto balance biológico y químico en tu setup marino.'}
          </p>
      ...
      <section className="border-t border-neutral-800 pt-8">
        <h2 className="text-sm font-bold mb-4 uppercase tracking-wider text-neutral-400">Productos relacionados</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {relacionados.map((item) => (
            <div key={item.id} className="bg-neutral-950 border border-neutral-800 rounded p-3 flex flex-col justify-between">
              <div className="w-full h-24 bg-neutral-900 rounded mb-2 flex items-center justify-center text-[10px] text-neutral-600">[ {item.imagen_url} ]</div>
              <div>
